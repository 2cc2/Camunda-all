...
async function deployOwnerModel(client: any): Promise<void> {
  ...
  const resources = [
    {
      name: 'owner-export-contract.bpmn',
      filePath: '../bpmn/owner-export-contract.bpmn'
    }
...
async function mockInboundMessages(client: any, orderId: string): Promise<void> {
  // 1) Transport -> Owner: ctn-to-owner
  ...
  // 2) Environment/Any -> Owner: expense-note-to-owner
...
async function main(): Promise<void> {
  ...
  if (mockInbound) {
    ...
    // Give the engine a moment to advance and complete the last service task.
    await new Promise((r) => setTimeout(r, 1200))
