...
async function getInstanceState(baseUrl: string, instanceKey: string): Promise<string> {
  try {
...
async function main(): Promise<void> {
  ...
  const rabbitConsumer = new RabbitMQConsumer(client)
  ...
  const ctnReal = await waitForVariable(CAMUNDA_REST_ADDRESS, instanceKey, 'ctnNumber', 15000)
  if (!ctnReal.found) {
  ...
  const expenseReal = await waitForVariable(CAMUNDA_REST_ADDRESS, instanceKey, 'expenseAmount', 15000)
  ...
  while (Date.now() - start < 30000) {
    const state = await getInstanceState(CAMUNDA_REST_ADDRESS, instanceKey)
    if (state === 'COMPLETED') {
