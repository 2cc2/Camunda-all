...
export function startShippingAgencyWorkers(client: CamundaRestClient) {
    const sendManifestWorker = client.createJobWorker<SAVariables, SAVariables>({
        ...
        jobHandler: async (job, log) => {
            ...
            const manifestFiledAt = new Date(Date.now() - 8 * 60 * 60 * 1000).toISOString()
            ...
            const payload = {
                ...
                vesselId,
                manifestVesselId: vesselId,
                manifestContainerId: containerId,
                manifestCargoDescription: 'Plush Toys',
                manifestQuantity: 500,
                manifestFiledAt,
            }
